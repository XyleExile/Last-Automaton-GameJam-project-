...
@export var knockback_force: float = 400.0  # Increased knockback force
...
@export var max_health: int = 500  # Maximum health for the big monster
@export var bullet_knockback_force: float = 150.0  # Knockback force when hit by bullets
@export var show_health_bar: bool = false  # Set to false to hide the health bar
...
var current_health: int = 500  # Current health, initialized to max_health
var is_alive: bool = true  # Track if the monster is alive
var health_bar: ProgressBar  # Reference to the health bar
...
func _ready():
	...
	# Health bar is disabled by default
	# Only create it if explicitly enabled
	if show_health_bar:
		create_health_bar()

# Create a health bar above the monster
func create_health_bar():
	# Create a ProgressBar node
	health_bar = ProgressBar.new()
	health_bar.min_value = 0
	health_bar.max_value = max_health
	health_bar.value = current_health
	health_bar.show_percentage = false
	health_bar.size = Vector2(50, 5)  # Small bar
	health_bar.position = Vector2(-25, -40)  # Position above the monster
	...
	fill_style.bg_color = Color(0.8, 0.2, 0.2)  # Red for health
...
func update_health_bar():
	if health_bar:
		...
		if health_percent > 0.6:
			fill_style.bg_color = Color(0.2, 0.8, 0.2)  # Green for high health
		elif health_percent > 0.3:
			fill_style.bg_color = Color(0.8, 0.8, 0.2)  # Yellow for medium health
		else:
			fill_style.bg_color = Color(0.8, 0.2, 0.2)  # Red for low health
...
func damage_player():
	...
	print("player damaged by big monster")
...
func take_damage(damage_amount: int, bullet_position: Vector2) -> void:
	...
	print("Big monster took damage! Current health: ", current_health)
...
func die() -> void:
	print("Big monster died!")
	is_alive = false
	
	# Update health bar only if it exists
	current_health = 0
	if show_health_bar and health_bar:
		update_health_bar()
		
		# Hide health bar after a short delay
