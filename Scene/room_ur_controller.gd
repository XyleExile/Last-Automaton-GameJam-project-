...
@export var door_start_2: Vector2i  # First tile of the door
@export var door_end_2: Vector2i  # Last tile of the door
...
@export var door_collision_2: CollisionShape2D  # Assign the door's collision shape
...
func remove_door():
	if tilemap:
		...
		for x in range(door_start_2.x, door_end_2.x + 1):  # Loop through Y for vertical doors
			var tile_pos = Vector2i(x, door_start_2.y)
			tilemap.set_cell(door_layer, tile_pos, -1)  # Remove the tile
	...
	if door_collision_2:
		door_collision_2.set_deferred("disabled", true)
		print("Door collision disabled!")
